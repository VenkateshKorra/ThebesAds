...
export class AccountSitesComponent implements AfterViewInit, OnInit {
  ...
  constructor(private http: HttpClient,private userService: UsersService, private router: Router, private messageService: MessageService) {
...
getMcmStatusBackgroundColor(mcm_status: string): any {
  ...
  stopAds(element: any) {
    console.log('StopAds clicked for site ID:', element.site_id);
    // alert('StopAds');
    const siteId = {
      site_id: element.site_id
    };
    this.http.post('http://localhost/ThebisAdsApp/DeActivatesite.php', siteId).subscribe(

    // console.log('Sending siteId:', siteId);
