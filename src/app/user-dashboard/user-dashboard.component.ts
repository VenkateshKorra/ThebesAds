    ...
    this.userService.getPollingRequest(Data).subscribe(
      (response) => {
        ...
        if (response.length <= 0) {
          ...
          tempVar = response[0].Request_Status;
          this.last_update_data_time = response[0].Completed_On == '0000-00-00 00:00:00' ? 'NA' : response[0].Completed_On;
          this.requested_date_time = response[0].Requested_On == '0000-00-00 00:00:00' ? 'NA' : response[0].Requested_On;
