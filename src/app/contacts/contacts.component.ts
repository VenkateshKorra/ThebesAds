...
export class ContactsComponent implements AfterViewInit, OnInit {
  ...
  openDialog(row: UserData) {
    const dialogRef = this.dialog.open(ContactDialogComponent, {
      width: '250%',
      data: row, // Pass row data to the dialog
    });
  ...
  rejectContact(row: UserData, reason: string): void {
    const updateData = {
      id: row.id,
  ...
  sendRejectionEmail(id: string | undefined, message: string): void {
    const params = {
      id: id,
      message: message
    };
