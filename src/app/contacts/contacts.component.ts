...
export class ContactsComponent implements AfterViewInit, OnInit {
  ...
  click=false;
  count = 1;
  displayedColumns: string[] = [ 'name', 'email', 'phone', 'country', 'status', 'invited', 'invite', 'reject', 'created_on', 'invited_on', 'created_by'];
  ...
  constructor(private userService: UsersService,  private router: Router, private messageService: MessageService, public dialog: MatDialog) {
  ...
  openRejectDialog(row: UserData) {
    const rejectdialogRef = this.dialog.open(ContactRejectComponent, {
      width: '250%',
      data: row,
    });
  ...
  rejectContact(id: any) {
    const updateData = {
      id: id,
      Reject: 'Y',
      // rejection_reason: reason,
      status: 'Rejected',
      // message: 'It wrong application'
    };
  
    this.userService.updateContacts(updateData).subscribe(
      (response) => {
        console.log("Successfully rejected", response);
        this.messageService.add({ severity: 'success', summary: 'Success', detail: 'Contact rejected', life: 5000 });
        // this.sendRejectionEmail(row.id, reason);
        this.show_contacts();
      },
      (error) => {
  ...
  send_reject_email(row: any, message: string): void {
    const params = {
      id: row.id,
      message: message
    };

    this.userService.sendRejectionEmail(params).subscribe(
      (response) => {
        console.log('Rejection email sent successfully', response);
        this.rejectContact(row.id);
        this.messageService.add({ severity: 'success', summary: 'Success', detail: 'Rejection email sent', life: 5000 });
      },
      (error) => {
        console.log('Error sending rejection email', error);
        this.messageService.add({ severity: 'error', summary: 'Error', detail: 'Error sending rejection email', life: 5000 });
      }
    );
  }
